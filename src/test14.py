...
class ImageClassifierAndNavigator():
    ...
    def image_callback(self, msg):
        try:
            ...
            min_distance_to_object = self.depth(msg)
    ...
    def print_robot_position(self, predicted_class, min_distance_to_object):
                ...
                if abs(delta_x) > abs(delta_y):
                    direction = "X"
                    final_x = current_x + min_distance_to_object
                elif abs(delta_y) > abs(delta_x):
                    direction = "Y"
                    final_y = current_y + min_distance_to_object
                else:
                    direction = "Unknown"
                rospy.loginfo(min_distance_to_object)
                # Log the robot position and direction
                rospy.loginfo(f"{predicted_class}, Robot position: x = {final_x}, y = {final_y}, Moving in direction: {direction}")    
                ...
                msg = "{:.2f},{:.2f},{}".format(final_x, final_y, predicted_class)
