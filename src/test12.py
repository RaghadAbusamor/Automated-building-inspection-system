...
class ImageClassifierAndNavigator():
    ...
    def depth(self, ):
        ...
        try:
            ...
            if depth_image is None:
                rospy.logwarn("Depth image data is None.")
                return None, "Depth image data is None."
    ...
    def image_callback(self, msg):
        try:
            ...
            if "cracks" in predicted_class.lower() or "structural" in predicted_class.lower():
                ...
                distance_to_object, depth_error = self.depth(msg)
                if distance_to_object is not None:
                        self.print_robot_position(predicted_class, distance_to_object)
                else:
    ...
    def print_robot_position(self, predicted_class, distance_to_object):
         # Retrieve current odometer data
        ...
        if abs(delta_x) > abs(delta_y):
            direction = "X"
            current_x = current_x + distance_to_object
        elif abs(delta_y) > abs(delta_x):
            direction = "Y"
            current_y = current_y + distance_to_object
        else:
            direction = "Unknown"
