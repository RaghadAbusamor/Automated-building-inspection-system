...
class ImageClassifierAndNavigator():
    def __init__(self):
        ...
        self.move_base_cancelled = False  # Flag to track if the navigation goal is cancelled
    ...
    def image_callback(self, msg):
        try:
            ...
            if "cracks" in predicted_class.lower() or "structural" in predicted_class.lower():
                ...
                # Start a thread to wait for a certain duration before allowing movement to continue
                wait_thread = threading.Thread(target=self.wait_for_defect_completion)
                wait_thread.start()
    ...
    def stop_navigation(self):
        if not self.move_base_cancelled:
            rospy.loginfo("Stopping navigation...")
            self.move_base.cancel_goal()
            self.move_base_cancelled = True

    def wait_for_defect_completion(self):
        rospy.sleep(self.wait_duration)
        self.defect_detected = False
        self.move_base_cancelled = False  # Reset the flag to allow navigation to resume
    ...
    def print_robot_position(self, predicted_class, min_distance_to_object):
        ...
        if abs(delta_x) > abs(delta_y):
            direction = "X"
            current_x += min_distance_to_object 
        ...
        msg = "{:.2f},{:.2f},{}".format(current_x, current_y, predicted_class)
