...
class ImageClassifierAndNavigator():
    def __init__(self):
        ...
        # rospy.Subscriber('/camera//image_raw', Image, self.get_closest_point)  # Corrected subscriber call
    ...
    def print_robot_position(self, predicted_class):
        ...
        msg = "{:.2f},{:.2f},{}".format(x, y, predicted_class)
            ...
            # Calculate closest point to the Kinect
        min_depth_x, min_depth_y = self.get_closest_point_coordinates()
        rospy.loginfo(f"Current robot position: min_depth_x = {min_depth_x}, min_depth_y = {min_depth_y}")
            # Append closest point information to the message
        # msg += ", Closest Point: X = {}, Y = {}, Distance = {} meters".format(
        #         min_depth_x, min_depth_y
        #     )

        # self.publish_mqtt("project_topic/automated_building_inspection_system/location", str(msg))

    def get_closest_point_coordinates(self):
        try:
            depth_image = None
            bridge = CvBridge()
            ...
            if depth_image is not None:
                ...
                min_depth = np.min(depth_image_filtered)
                if np.isinf(min_depth):
                    return None, None
                else:
                    min_depth_coords = np.unravel_index(np.argmin(depth_image_filtered), depth_image_filtered.shape)
                    min_depth_x = min_depth_coords[1]
                    min_depth_y = min_depth_coords[0]
                    return min_depth_x, min_depth_y
