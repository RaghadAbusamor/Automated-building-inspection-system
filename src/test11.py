...
class ImageClassifierAndNavigator():
    ...
    def image_callback(self, msg):
        try:
            ...
            # if "cracks" in predicted_class.lower() or "structural" in predicted_class.lower():
                # rospy.sleep(5)
            depth_x, depth_y = self.depth()
            if depth_x is not None and depth_y is not None:
                self.print_robot_position(predicted_class, depth_x, depth_y)
            else:
    ...
    def print_robot_position(self, predicted_class, depth_x, depth_y):
        odom_data = rospy.wait_for_message("/odom", Odometry)
        x = odom_data.pose.pose.position.x
        y = odom_data.pose.pose.position.y

        rospy.loginfo(f"robot position: depth_x = {depth_x}, depth_y = {depth_y}")
